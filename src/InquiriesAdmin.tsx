...
export default function InquiriesAdmin() {
  ...
  const activeId = selectedId && items.some((item) => item.id === selectedId) ? selectedId : items[0]?.id;
  ...
  return (
    <div className="presence-admin presence-inquiries-admin">
      ...
      {items.length > 0 && <div className="inquiry-master-detail">
        <div className="inquiry-inbox-list" aria-label="Consultas de esta etapa">
          {items.map((item) => <button key={item.id} type="button" aria-pressed={activeId === item.id} className={activeId === item.id ? "active" : ""} onClick={() => setSelectedId(item.id)}>
            <span><strong>{item.name}</strong><small>{item.interest} · {statusLabels[item.status]}</small></span>
            <time dateTime={item.createdAt}>{new Date(item.createdAt).toLocaleDateString("es-AR")}</time>
          </button>)}
        </div>
      <div className="inquiry-list presence-inquiry-list" aria-label="Detalle de la consulta seleccionada">
