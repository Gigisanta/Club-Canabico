...
const appRoutes = [
  "/app", "/app/ventas", "/app/inventario", "/app/socios", "/app/gastos",
  "/app/finanzas", "/app/responsables", "/app/reportes", "/app/vidriera",
  "/app/consultas", "/app/configuracion",
];
const captureRoutes = new Set(["/", "/productos", "/app", "/app/ventas", "/app/inventario", "/app/socios", "/app/vidriera", "/app/consultas"]);
const rows = [];
...
try {
  for (const size of sizes) {
    ...
    async function inspect(path, area) {
      ...
      if (area === "app") await page.locator(".page-loading").first().waitFor({ state: "hidden", timeout: 12_000 }).catch(() => {});
