...
Estado: candidato técnico local en `codex/bombo-ui`; **vista previa local, sin aprobación editorial ni despliegue público**. Sigue vigente `salida-vista-previa.md`.

## Rediseño del panel interno · 25 de septiembre de 2026

- `/app/` abre Resumen de hoy y `/app/decisiones` conserva el centro de decisiones. La navegación centraliza seis hubs, destinos y permisos para barra, búsqueda, encabezado y móvil. El control de cada hub expone encabezado, `aria-expanded`, `aria-controls` y una región asociada; solo uno queda abierto.
- Ventas y Socios paginan 12 y 10 registros por pantalla, respectivamente, conservando la navegación a páginas posteriores. En móvil, la altura de la vista inicial de Ventas bajó de 17.326 a 2.856 px y la de Socios de 9.364 a 2.647 px. Stock abre en tabla compacta en escritorio y lista compacta en móvil; la vista de tarjetas sigue disponible.
- `npm run check` con `TEST_DATABASE_URL` local dedicado: typecheck, **66/66 pruebas**, ninguna omitida y build correcto. `npm run test:e2e` en un esquema PostgreSQL desechable: **20/20 pruebas**; esquema eliminado al terminar. Incluye alta de lote y socio, venta, ficha, importación, roles, búsqueda, teclado, menú móvil y rutas profundas.
- `UI_AUDIT_OUTPUT=.local/ui-qa/redesign-final node scripts/ui-audit.mjs`: **125 comprobaciones de rutas y anchos, 0 errores**. Capturas previas del panel en `.local/ui-qa/after/` y posteriores en `.local/ui-qa/redesign-final/`. Se revisaron 1440, 768, 720 (ancho CSS equivalente a 200 % desde 1440), 390 y 320 px; sin desbordamiento horizontal en las rutas medidas.
- El navegador automatizado verificó foco, teclado y estructura ARIA. Queda pendiente una sesión humana con lector de pantalla real; el ancho CSS equivalente no sustituye comprobar el zoom del navegador en dispositivos finales.

## Verificación anterior de marca y web pública
